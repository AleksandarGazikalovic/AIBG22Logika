package aibg.logika.Game;

public class GameParameters {
    public static final int STARTING_HEALTH = 500; //menjati
    public static final int STARTING_POWER = 100; //menjati

    public static final int RANGE = 3;
    public static final int EFFECT_DURATION_BLACKHOLE = 3;
    public static final int TIMER_DURATION = 3 * 60 * 1000;
    public static final int ILLEGAL_MOVES = 10;
    public static final int EXP_ON_KILL = 250;//menjati
    public static final int EXP_GATHER = 30; //menjati
    public static final int EXP_BY_TIME = 5; //menjati
    public static final int MAX_LEVEL = 5;
    public static final int EXP_TO_LVL_UP = 500; //menjati
    public static final int LEVEL_2_POWER_INCREMENT = 10; //menjati
    public static final int LEVEL_3_POWER_INCREMENT = 15; //menjati
    public static final int LEVEL_4_POWER_INCREMENT = 20; //menjati
    public static final int LEVEL_5_POWER_INCREMENT = 30; //menjati
    public static final int FENCE_HEALTH = 300;
    public static final int BOSS_POWER = 100;
    //revive points
    //shields
}
